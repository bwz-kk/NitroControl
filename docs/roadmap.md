...
- Writing `platform_profile` is the real control surface (no `pwm*` write path exists): `low-power`, `quiet`, `balanced`, `balanced-performance` all write successfully and produce measured, causal fan-speed changes (largest: 2736→4030 RPM going to `balanced-performance`). `performance` fails with an EIO write error — a partial DMI-quirk match, not a full one (this model isn't in `acer_wmi`'s quirk table; interpretation detail in `hardware.md`).
