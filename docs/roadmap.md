...
- Decide & implement a persistent config path (`modprobe.d` boot param) so `predator_v4=1` isn't a manual per-boot reload — a system-configuration change. Current lean: NitroControl itself should stay detect-only (never load/reload kernel modules or write boot config on its own) and document this as an optional manual opt-in step for users who want it — avoids adding a privileged system-config-mutation feature to v1 and keeps SAFE-002's "no arbitrary/raw hardware writes" spirit intact; needs confirming as an actual decision, not just a lean, before it's treated as settled.
- Design (SPECIFY) an Acer-firmware power-profile control surface (the `platform_profile` write path found in the experiment) and how "4/5 profiles work, `performance` always EIOs" surfaces honestly through `CapabilityState`/the CLI/the GUI, and how it relates to the existing OS-level `PowerProfileProvider` (same trait shape reused via a new `PowerProfilesBackend` impl backed by sysfs instead of D-Bus looks like the natural fit, but this is a real interface decision, not yet made) — a new shape, not yet modeled by any existing provider.
- TDD-implement that Acer-firmware profile control behind the usual `SysfsReader` seam, on its own branch+PR per the established workflow.
