...
- **Interpretation of the `performance` EIO**: this machine's DMI is not in `acer_wmi`'s quirk table for `WMID_GUID4` (see above — only PH315-53/PH16-72/PT14-51/AN515-58 are upstream-confirmed), so `predator_v4=1` forces the generic quirk profile rather than a table entry matched to this exact model/firmware. The most likely explanation is an ACPI method (`_PPC`/`_OSC`-equivalent WMI call) that the forced-generic profile maps to a request this firmware doesn't accept for `performance` specifically, while accepting it for the other four values — i.e. a **partial, not full**, quirk match. Not root-caused further this pass (would need `predator_v4=1` reloaded again with `dyndbg`/WMI tracing — deferred, not required to record this result).
