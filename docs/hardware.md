...
- **Update (issue #25)**: NitroControl now acts on this finding instead of just documenting it. `ProfileInfo.known_unsupported` (`nitroctl-core/src/power_profile.rs`) marks `performance` whenever it's backed by a real ACPI `platform_profile` driver — always true for `AcerPlatformProfileBackend`, and true for the PPD backend once it adopts that driver (`PlatformDriver == "platform_profile"`) rather than the CpuDriver-only `amd_pstate` path. `PowerProfilesDaemon::set_profile()` rejects a known-unsupported name before attempting the write (`ProfileError::KnownUnsupportedProfile`), so `nitroctl profile set performance` / `nitroctl acer-profile set performance` now fail fast with an explanation instead of a raw `-EIO`. The GUI's main Power-tab picker grays out the `performance` pill with the same explanation as a tooltip.
...
| Power profile (Acer firmware) | HARDWARE_DEPENDENT | HARDWARE_DEPENDENT | HARDWARE_DEPENDENT (4/5 values; `performance` rejected client-side as a known permanent limitation, issue #25 — never attempted) | `/sys/firmware/acpi/platform_profile`, only present when `acer_wmi` is loaded with `predator_v4=1` (not this machine's default) | Yes — `list`/`get`/`set` all exercised live during the M5 experiment; 4/5 profile values write successfully and change real fan RPM, `performance` fails EIO; node is entirely absent (`UNSUPPORTED`) under default boot config |
