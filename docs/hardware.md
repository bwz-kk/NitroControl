...
| Battery charge limit | UNSUPPORTED (M6/FR-008 implementation not started — SPECIFY done) | UNSUPPORTED | UNSUPPORTED | out-of-tree, adopted for M6: [`bwz-kk/acer-wmi-battery`](https://github.com/bwz-kk/acer-wmi-battery) (fork of `frederik-h/acer-wmi-battery` with the heap-read fix below), `/sys/bus/wmi/drivers/acer-wmi-battery/health_mode`, only present when that module is loaded (NitroControl never loads it on its own) | Yes — hardware capability confirmed present and working via the discovery test; `nitroctl-core` implementation not yet written, matrix stays UNSUPPORTED per COMPAT-002 until it is |
